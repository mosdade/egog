...
// Глобальные настройки для всех функций
 ...
 */
exports.uploadQuestions = onRequest({
  cors: true,
  // Требует план Blaze, иначе используйте переменные окружения
  secrets: ["SECRET_KEY"],
}, async (req, res) => {
  // Проверка авторизации через секретный ключ
  // Приоритет: Firebase Secrets > переменные окружения
  const secretKey = process.env.SECRET_KEY || process.env.SECRET_KEY_ENV;
  ...
  try {
    ...
    if (!subjectCode || !questions || !Array.isArray(questions)) {
      res.status(400).json({
        error: "Invalid request. " +
          "Required: subjectCode (string), questions (array)",
      });
      return;
    }
    ...
    if (!validSubjects.includes(subjectCode)) {
      res.status(400).json({
        error: `Invalid subjectCode. ` +
          `Valid codes: ${validSubjects.join(", ")}`,
      });
      return;
    }

    logger.info(
        `Starting upload of ${questions.length} questions for ${subjectCode}`,
    );
    ...
    for (let i = 0; i < questions.length; i += batchSize) {
      ...
      for (const question of batchQuestions) {
        try {
          ...
          logger.error(`Error preparing question ${question.id}:`, error);
          errors++;
        }
      }
      ...
      try {
        ...
        logger.info(`Uploaded batch: ${uploaded}/${questions.length}`);
      } catch (error) {
 ...
 */
exports.parseAndUpload = onRequest({
  cors: true,
  // Требует план Blaze, иначе используйте переменные окружения
  secrets: ["SECRET_KEY"],
}, async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      success: true,
      message: "Parser integration needed. " +
        "Use uploadQuestions endpoint with parsed data.",
      note: "To integrate parser, add parsing logic here " +
        "or call external parsing service.",
    });
  } catch (error) {
    logger.error("Error in parseAndUpload:", error);
    res.status(500).json({
      error: "Internal server error",
      message: error.message,
    });
 ...
 */
exports.getQuestionsStats = onRequest({
  cors: true,
}, async (req, res) => {
  try {
    const stats = {};
